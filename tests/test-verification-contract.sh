...
! grep -q -- '--clobber' "${workflow}" ||
    fail "release assets can still overwrite an existing release"
...
grep -qx 'EXPECTED_RELEASE_TAG=v1.2.6-r1' "${versions}" ||
    fail "expected immutable release tag is missing"
...
missing_output=$("${verifier}" x86_64-unknown-linux-musl /nonexistent \
    16.1.0 1.2.6 ld-musl-x86_64.so.1 2>&1)
missing_status=$?
set -e
